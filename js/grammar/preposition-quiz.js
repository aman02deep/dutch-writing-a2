...
function startQuiz() {
    ...
    const allQuestions = prepositionQuizData[selectedLevel];
...
function showHint() {
    ...
    hintDisplay.innerHTML = `<strong>💡 Hint:</strong> ${question.hint} (category: <em>${categoryName}</em>)`;
...
function checkAnswer() {
    ...
    if (userAnswer === correctAnswer) {
        ...
        feedback.innerHTML = `
            ...
            <div class="correct-answer">The correct answer is: <strong>${question.answer}</strong></div>
        `;
        feedback.style.display = 'block';
    }
...
function showResults() {
    ...
    let message = '';
    if (percentage >= 90) {
        message = '🎉 Excellent! You have mastered these prepositions!';
    } else if (percentage >= 70) {
        message = '👍 Great job! Keep practicing to improve further.';
    } else if (percentage >= 50) {
        message = '💪 Good effort! Review the prepositions and try again.';
    } else {
