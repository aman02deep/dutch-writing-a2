...
async function showHint() {
    ...
    // Show static hint immediately
    hintDisplay.innerHTML = `
        <strong>💡 Hint:</strong> ${question.hint} (infinitive: <em>${question.infinitive}</em>)
        <div id="ai-hint-container" style="margin-top: 8px; border-top: 1px dashed #ccc; padding-top: 8px;">
            <span class="loading-dots">Asking AI for more help...</span>
        </div>
    `;
    hintDisplay.classList.add('show');

    // Get AI Hint
    const aiHint = await aiService.getVerbHint(
        question.infinitive,
        selectedLevel === 'a1' ? 'Present' : 'Past', // Approximation, ideally data has tense
        question.sentence
    );

    const aiContainer = document.getElementById('ai-hint-container');
    if (aiContainer) {
        aiContainer.innerHTML = `<strong>🤖 AI Clue:</strong> ${aiHint}`;
    }
}
...
async function checkAnswer() {
    ...
    if (userAnswer === correctAnswer) {
        ...
        // Incorrect answer - Ask AI Why
        input.classList.add('incorrect');
        feedback.classList.add('incorrect');

        // Show loading
        feedback.innerHTML = `❌ <strong>Incorrect.</strong> <span class="loading-dots">AI is analyzing your mistake...</span>`;
        feedback.style.display = 'block';

        const explanation = await aiService.explainVerbError(
            question.infinitive,
            selectedLevel === 'a1' ? 'Present' : 'Past',
            userAnswer,
            question.answer,
            question.sentence
        );
        ...
        feedback.innerHTML = `
            ...
            <div class="ai-explanation" style="margin-top:10px; padding:10px; background:rgba(255,255,255,0.9); border-radius:8px; border-left: 4px solid #e74c3c;">
                <strong>🤖 AI Feedback:</strong><br>
                ${explanation}
            </div>
            <div class="correct-answer" style="margin-top:5px;">Correct answer: <strong>${question.answer}</strong></div>
        `;
